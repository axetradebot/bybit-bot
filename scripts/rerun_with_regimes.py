"""
Rerun pipeline: save old results, recompute indicators, run backtests,
then compare before/after.
"""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from sqlalchemy import create_engine, text
from src.config import settings


def main():
    engine = create_engine(settings.sync_db_url)

    print("[1/4] Saving old backtest results for comparison...")
    with engine.begin() as conn:
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS trades_log_pre_regime AS
            SELECT * FROM trades_log WHERE is_backtest = TRUE
        """))
        old_count = conn.execute(text(
            "SELECT COUNT(*) FROM trades_log_pre_regime"
        )).scalar()
        print(f"  Saved {old_count} pre-regime trades")

    print("[2/4] Clearing old backtest trades...")
    with engine.begin() as conn:
        deleted = conn.execute(text(
            "DELETE FROM trades_log WHERE is_backtest = TRUE"
        )).rowcount
        print(f"  Deleted {deleted} old backtest rows")

        conn.execute(text("DELETE FROM strategy_performance"))
        print("  Cleared strategy_performance")

    print("[3/4] Recomputing indicators (MACD signal=3 fix)...")
    import subprocess
    for sym in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
        print(f"  -> indicators for {sym} ...", end=" ", flush=True)
        result = subprocess.run(
            [sys.executable, "src/indicators/compute_all.py",
             "--symbol", sym],
            capture_output=True, text=True, encoding="utf-8",
        )
        if result.returncode != 0:
            print(f"FAILED: {result.stderr[-300:]}")
            return
        print("OK")
    print("  All indicators recomputed")

    print("[4/4] Running backtests for all strategies...")
    strategies = [
        "bb_squeeze", "multitf_scalp", "rsi_divergence",
        "vwap_reversion", "volume_delta_liq", "regime_adaptive",
    ]
    for strat in strategies:
        for sym in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
            print(f"  -> {strat} / {sym} ...", end=" ", flush=True)
            r = subprocess.run(
                [sys.executable, "src/backtest/simulator.py",
                 "--strategy", strat, "--symbol", sym],
                capture_output=True, text=True, encoding="utf-8",
            )
            last_line = (r.stdout.strip().split("\n")[-1]
                         if r.stdout.strip() else "no output")
            if r.returncode != 0:
                err_tail = r.stderr.strip().split("\n")[-1] if r.stderr.strip() else ""
                print(f"FAIL: {err_tail}")
            else:
                print(last_line)

    print("\n" + "=" * 80)
    print("UPDATING strategy_performance...")
    subprocess.run(
        [sys.executable, "scripts/update_strategy_performance.py"],
        capture_output=True, text=True, encoding="utf-8",
    )

    print("\n" + "=" * 80)
    print("COMPARISON: Before vs After regime filters")
    print("=" * 80)
    with engine.connect() as conn:
        print("\n--- BEFORE (pre-regime) ---")
        old_rows = conn.execute(text("""
            SELECT
                strategy_combo,
                symbol,
                COUNT(*) AS trades,
                ROUND(AVG(win_loss::int) * 100, 1) AS wr,
                ROUND(AVG(pnl_pct) * 100, 4) AS avg_pnl,
                ROUND(SUM(pnl_usd), 2) AS total_pnl
            FROM trades_log_pre_regime
            GROUP BY strategy_combo, symbol
            HAVING COUNT(*) >= 10
            ORDER BY AVG(pnl_pct) DESC
        """)).fetchall()

        print(f"{'Combo':<55} {'Sym':<10} {'Trades':>7} {'WR%':>6} {'AvgPnl%':>8} {'TotalPnL':>10}")
        print("-" * 100)
        for r in old_rows:
            combo_str = str(r[0])[:54]
            print(f"{combo_str:<55} {r[1]:<10} {r[2]:>7} {r[3]:>5.1f}% {r[4]:>7.4f}% ${r[5]:>9.2f}")

        print("\n--- AFTER (with regime filters + MACD fix) ---")
        new_rows = conn.execute(text("""
            SELECT
                strategy_combo,
                symbol,
                COUNT(*) AS trades,
                ROUND(AVG(win_loss::int) * 100, 1) AS wr,
                ROUND(AVG(pnl_pct) * 100, 4) AS avg_pnl,
                ROUND(SUM(pnl_usd), 2) AS total_pnl
            FROM trades_log
            WHERE is_backtest = TRUE
            GROUP BY strategy_combo, symbol
            HAVING COUNT(*) >= 10
            ORDER BY AVG(pnl_pct) DESC
        """)).fetchall()

        print(f"{'Combo':<55} {'Sym':<10} {'Trades':>7} {'WR%':>6} {'AvgPnl%':>8} {'TotalPnL':>10}")
        print("-" * 100)
        for r in new_rows:
            combo_str = str(r[0])[:54]
            print(f"{combo_str:<55} {r[1]:<10} {r[2]:>7} {r[3]:>5.1f}% {r[4]:>7.4f}% ${r[5]:>9.2f}")

        # Summary comparison
        print("\n--- AGGREGATE COMPARISON ---")
        old_agg = conn.execute(text("""
            SELECT COUNT(*), ROUND(AVG(win_loss::int)*100,1),
                   ROUND(AVG(pnl_pct)*100,4), ROUND(SUM(pnl_usd),2)
            FROM trades_log_pre_regime
        """)).fetchone()
        new_agg = conn.execute(text("""
            SELECT COUNT(*), ROUND(AVG(win_loss::int)*100,1),
                   ROUND(AVG(pnl_pct)*100,4), ROUND(SUM(pnl_usd),2)
            FROM trades_log WHERE is_backtest = TRUE
        """)).fetchone()

        print(f"  BEFORE: {old_agg[0]:>7} trades | WR={old_agg[1]:>5.1f}% | "
              f"AvgPnl={old_agg[2]:>7.4f}% | Total=${old_agg[3]:>12.2f}")
        print(f"  AFTER:  {new_agg[0]:>7} trades | WR={new_agg[1]:>5.1f}% | "
              f"AvgPnl={new_agg[2]:>7.4f}% | Total=${new_agg[3]:>12.2f}")

    print("\nDone! Open the dashboard to explore the results.")


if __name__ == "__main__":
    main()
